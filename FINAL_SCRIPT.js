 ...
 * Дата: 2025-12-22
 * 
 * Собирает 50+ поведенческих метрик для ML-прогнозирования конверсий
 * Интеграция: Tilda + Supabase + Яндекс.Метрика
 */
...
(function () {
    ...
    window.TildaAnalytics = {
        ...
        async init(config) {
            try {
                ...
                this.config = {
                    ...
                    customEvents: {},
                    excludeSelectors: [],
                    yandexMetrikaId: null, // ID счетчика Яндекс.Метрики
                    ...config
                };
                ...
                // Initialize Yandex.Metrika integration
                if (this.config.yandexMetrikaId) {
                    this._initYandexMetrika();
                }

                this._log('✅ Tilda Analytics initialized', {
                    userId: this.userId,
        ...
        _initEventListeners() {
            ...
            // Initialize block observers (pricing, reviews, etc.)
        ...
        _handleClick(event) {
            ...
            if (this._clickTimestamps.length >= 3) {
                ...
                this._log('⚡ Rage click detected');
            }

            // Check for dead clicks (non-interactive elements)
        ...
        _handleScroll() {
            ...
            if (this._scrollHistory.length > 0) {
                ...
                // Scroll pause detection (velocity very low)
                if (velocity < 50) {
        ...
        _handleMouseOut(event) {
            ...
            // Calculate hover duration on images
        ...
        _initFormTracking() {
            ...
            // Tilda Ajax forms (specific handling)
            if (window.jQuery) {
        ...
        async _handleFormSubmit(event, form) {
            ...
            for (let [key, value] of formData.entries()) {
                // Only collect email, phone, name (не пароли!)
            ...
            // Reset form abandonment
            this.metrics.form_abandonment_rate = false;
        },

        async _handleTildaFormSuccess(formData) {
            this._log('✅ Tilda form success:', formData);

            // Extract contact data from Tilda form
            const contactData = {};
        ...
        async _updateUserContact(contactData) {
            try {
                ...
                if (error) throw error;
                this._log('📞 Contact data updated:', contactData);
            } catch (error) {
        ...
        // =============================================================================
        // BLOCK OBSERVERS (Pricing, Reviews, etc.)
        // =============================================================================

        _initBlockObservers() {
            // Pricing block observer
        ...
        _handleJSError(event) {
            ...
            this._log('❌ JavaScript error:', event.message);
        },
            ...
            Object.defineProperty(window, 'ym', {
                ...
                set(value) {
                    ...
                    if (!proxyInstalled && typeof value === 'function') {
                        ...
                        self._log('✅ Yandex.Metrika lazy proxy installed');
                    }
                },
                configurable: true
            });
        ...
        async _flushEvents() {
            ...
            try {
                ...
                // Push back to buffer for retry
                this._eventBuffer.unshift(...events);
            }
        },

        // =============================================================================
        // BEFORE UNLOAD (Final metrics save)
        // =============================================================================
        ...
        _handleBeforeUnload() {
            ...
            this._log('👋 Session ending, metrics saved');
        },
            ...
            try {
                await this.supabase
                    ...
                    .update({
                        ...
                        pages_viewed: 1, // can be tracked if SPA
