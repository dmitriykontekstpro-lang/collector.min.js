...
const App = {
    ...
    _initYandexMetrika() {
        ...
        window.ym = (...args) => {
            if (args[1] === 'reachGoal') {
                const goalId = args[2];
                this.trackGoal(goalId, args[3]); // Исправлено: args[3] это params
    ...
    trackGoal(id, params) { 
        this.state.goals.add(id); 
        this.trackEvent("yandex_goal"); 
        this._saveState(); 
        this._log("🎯 Goal:", id);
    },

    // --- 3. BEHAVIOR SENSORS (v7.2 Logic) ---
    _initBehaviorSensors() {
        ...
        document.addEventListener('click', (e) => {
            ...
            // Exit Tracker Updates
            this._updateHoverEl(e.target);
        }, true);

        // Visibility & Exit Tracker
    ...
    // --- 5. SYNC ---
    _initTick() {
        this._tickTimer = setInterval(() => { if(!document.hidden && this.state.currentSessionId) {
    ...
    async _syncToSupabase() {
        ...
        this._log(`⭐ Sync ${this.version}`, data);
